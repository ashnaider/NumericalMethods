"""
Функции для численного решения
задачи Коши для обыкновенного дифференциального
уравнения методом Эйлера-Коши

    * Euler_Cauchy_method - метод Эйлера - Коши

Автор: Шнайдер Антон
"""

from diff_equations import *  # ДУ

import os, sys
sys.path.append(os.path.dirname(os.path.dirname(os.path.realpath(__file__))))

from utils import *  # служебные функции

# библиотека для построение графиков
import matplotlib.pyplot as plt  
import numpy as np  # для работы с массивами
import pandas as pd # для работы с табличными данными 
from dataclasses import make_dataclass
# для создания классов данных 


def Euler_Cauchy_method(x0, y0, a, b, h):
    # создаём таблицу и
    # заполняем первую строку таблицы начальными значениями

    Iteration = make_dataclass("Iteration", 
                    [
                        ("xi", float), 
                        ("yi", float), 
                        ("yi_sq", float),
                        ("delta_yi", float),
                        ("actual_yi", float),
                        ("eps", float)
                    ])

    df = pd.DataFrame([Iteration(x0, y0, 0, 0, diff_eq_solution(x0), 
                        abs(y0 - diff_eq_solution(x0)))])
    
    # получаем иксы на интервале [a;b+h]
    # с шагом h
    x = np.arange(a, b+h, h)

    i = 1
    for xi in x[i:]:
        # получаем предыдущие значения x и y
        # из таблицы 
        y_prev = df.iloc[i-1]["yi"]
        x_prev = df.iloc[i-1]["xi"]

        # вычисляем правую часть ДУ f(x, y)
        fi = f(x_prev, y_prev)
        yi_sq = y_prev + h * fi  # получаем игрек с волной

        k = 7  # вывод информации на экран
        print(f"~y{i} = y{i-1} + h * f(x{i-1}, y{i-1})"
                  f" = {y_prev:.{k}f} + {h:.{k}f} * "
                  f"({x_prev:.{k}f} + {y_prev:.{k}f})^2"
                  f" = {yi_sq:.{k}f}")

        # вычисляем дельта игрек
        delta_y = 0.5 * h * (f(x_prev, y_prev) + f(xi, yi_sq))
        yi = y_prev + delta_y  # получаем игрек для i-й точки

        # вывод информации
        print(f" y{i} = y{i-1} + 0.5h[f(x{i-1}, y{i-1}) + f(x{i}, ~y{i})]"
                  f" = {y_prev:.{k}f} + 0.5 * {h:.{k}f} * "
                  f"({f(x_prev, y_prev):.{k}f} + {f(xi, yi_sq):.{k}f})"
                  f" = {yi:.{k}f}\n")

        # точное аналитическое знаачение
        actual_y = diff_eq_solution(xi)
        # добавляем новую строку в таблицу
        eps = abs(actual_y - yi)
        df = df.append([Iteration(xi, yi, yi_sq, delta_y, actual_y, eps)], 
                                                ignore_index=True)
        i+=1

    print(df, "\n")  # печатаем таблицу
    return df  # возвращаем таблицу


if __name__ == "__main__":
    # читаем данные из файла
    data = read_data("Diff_equations/data/h=0.05.txt", ["a", "b", "h", "x0", "y0"])
    x0 = data["x0"][0]
    y0 = data["y0"][0]
    a = data["a"][0]
    b = data["b"][0]

    h1 = 0.1  
    h2 = 0.05

    # рассчитываем таблицу для h=0.1 и h=0.05
    print(f"h = {h1}")
    df = Euler_Cauchy_method(x0, y0, a, b, h1)
    # сохраняем таблицу как csv файл
    df.to_csv(f"Diff_equations/output/Euler-Cauchy_h={h1}.csv", float_format='%1.5f', sep=";")

    print(f"h = {h2}")
    df2 = Euler_Cauchy_method(x0, y0, a, b, h2)
    # сохраняем таблицу как csv файл
    df2.to_csv(f"Diff_equations/output/Euler-Cauchy_h={h2}.csv", float_format='%1.5f', sep=";")

    plt.xlabel("X")  # обозначаем оси
    plt.ylabel("Y")
    plt.title("Задача Коши методом Эйлера-Коши\ny' = (y + x)^2\ny(0)=0")
    plt.plot(df.xi, df.yi, label=f"h={h1}")  # строим 
    plt.plot(df2.xi, df2.yi, label=f"h={h2}") # графики
    plt.scatter(df.xi, df.yi)  # отмечаем узловые точки
    plt.scatter(df2.xi, df2.yi, label="узловые точки")
    plt.legend()
    plt.grid()  # сохраняем изображение
    # plt.savefig("img/Euler_Couchy_method_diff_eq.png")
    plt.show()  # отображаем графики